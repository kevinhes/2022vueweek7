...
export default {
  ...
  methods: {
    updataProduct(id) {
      ...
      if (this.isNew === true) {
        this.$http.post(`${process.env.VUE_APP_API}/v2/api/${process.env.VUE_APP_PATH}/admin/product`, obj)
          .then((res) => {
        ...
        this.$http.put(`${process.env.VUE_APP_API}/v2/api/${process.env.VUE_APP_PATH}/admin/product/${id}`, obj)
          .then((res) => {
            alert(res.data.message);
            this.productModal.hide();
            this.$emit('get-data');
